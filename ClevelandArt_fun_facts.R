...
cleveland_data <-
  read.csv("~/R/DATA/cleveland-data.csv",
           encoding = "UTF-8",
...
for (i in 1:nrow(funfactsonly)) {
  ...
  if (funfactsonly$thumb_OK[i] == TRUE) {
    ...
    this_image = implot(this_image,
                        text(
                          1263 / 2,
                          1263 / 4 * 3,
                          paste(paste(
                            strwrap(
                              funfactsonly$fun_fact[i],
                              width = 55,
                              indent = 0,
                              exdent = 0,
                              prefix = "",
    ...
    if (nchar(caption) > 65) {
      caption = paste(paste(substr(title, 0, 60), ellipsis, sep = ""),
                      date, sep = ", ")
    }
    if (!creator == "") {
    ...
    save.image(this_image,
               paste(folder_twitter, funfactsonly$X.U.FEFF.id[i], ".jpg", sep = ""))
  }
}
...
#First line
tweet_text = paste(
  "This bot chronologically tweets fun facts about objects at the Cleveland Museum of Art until today next year.",
  tweet_line_break,
  "Not affiliated with but using open data from @clevelandart",
  tweet_line_break,
  "A project by @HxxxKxxx",
  tweet_line_break,
  "#DigitalArtHistory @auto_chirp",
  sep = ""
)
image_url = paste(tweet_imgurl, "banner.jpg", sep = "")
tweet_tsv[1, ] = list(as.character(tweet_date),
                      "07:20",
                      tweet_text,
                      image_url,
                      "",
...
for (i in 1:nrow(funfactsonly)) {
  if (funfactsonly$thumb_OK[i] == TRUE) {
    ...
    image_url = paste(tweet_imgurl, funfactsonly$X.U.FEFF.id[i], ".jpg", sep =
                        "")
    tweet_tsv[nrow(tweet_tsv) + 1, ] = list(as.character(tweet_date),
                                            tweet_time,
                                            tweet_text,
                                            image_url,
                                            "",
...
#Last line
tweet_text = paste(
  "That's it. The fun facts are over. Thank you @clevelandart",
  tweet_line_break,
  "This has been a project by @HxxxKxxx",
  tweet_line_break,
  "#DigitalArtHistory #TwitterOff",
  sep = ""
)
image_url = paste(tweet_imgurl, "banner.jpg", sep = "")
tweet_tsv[nrow(tweet_tsv) + 1, ] = list(as.character(tweet_date - 1),
                                        "07:31",
                                        tweet_text,
                                        image_url,
                                        "",
